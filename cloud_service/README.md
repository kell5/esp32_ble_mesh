...
该服务把现有门铃与 ESP-WIFI-MESH MQTT 消息收敛为统一设备注册表和设备影子。首版采用 FastAPI + SQLite，本地即可运行；开启 MQTT 后兼容现有固件和 App 主题，不要求先改硬件协议。
...
当前 `user_id` 是设备归属模型，不等同于完整登录系统。公网部署前还需接入正式用户认证、TLS、broker ACL，以及固件侧 `version/message_id` 和 MQTT LWT；房间、场景、自动化、OTA 与门铃事件索引在后续切片实现。
