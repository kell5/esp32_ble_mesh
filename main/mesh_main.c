...
void esp_mesh_p2p_tx_main(void *arg)
{
    ...
    while (is_running) {
        if (!esp_mesh_is_root()) {
            ...
            ESP_LOGI(MESH_TAG, "root ready for MQTT, mqtt:%s", s_mqtt_started ? "started" : "stopped");
        }
...
static void mqtt_handle_control(const char *payload, int len)
{
    if (!payload || len <= 0 || !esp_mesh_is_root()) {
        return;
    }

    if ((len == 2) && (strncmp(payload, "on", 2) == 0 || strncmp(payload, "ON", 2) == 0)) {
        mesh_broadcast_ctl(&light_on);
        mqtt_publish_status("cmd_on");
    } else if ((len == 3) && (strncmp(payload, "off", 3) == 0 || strncmp(payload, "OFF", 3) == 0)) {
        mesh_broadcast_ctl(&light_off);
        mqtt_publish_status("cmd_off");
    } else {
        ESP_LOGW(MESH_TAG, "unknown mqtt cmd: %.*s", len, payload);
    }
}
                               ...
                               int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_DATA:
        if (event->topic_len == (int)strlen(MQTT_SUBSCRIBE_TOPIC) &&
            strncmp(event->topic, MQTT_SUBSCRIBE_TOPIC, event->topic_len) == 0) {
            mqtt_handle_control(event->data, event->data_len);
        }
        break;
